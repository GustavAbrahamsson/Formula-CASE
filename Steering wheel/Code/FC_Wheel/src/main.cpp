...
Arduino_DataBus *bus = new Arduino_ESP32SPI(-1, SS, SCK, MOSI, -1, 0);

//Arduino_DataBus *bus = new Arduino_SWSPI(-1, SS, SCK, MOSI, -1);

/* More display class: https://github.com/moononournation/Arduino_GFX/wiki/Display-Class */
Arduino_GFX *screen = new Arduino_ILI9341(bus, -1, 0 /* rotation */, false /* IPS */);
...
void draw_rpm_gauge(float percentage)
{
  ...
  if (old_gauge_pos < red_end)
    screen->fillRect(red_start, 0, red_end - old_gauge_pos, RPM_GAUGE_HEIGHT, 0xF800);
  if (end_at == "red")
  {
    return;
  }
